...
1.  **Business Understanding:**
    ...
    VI. What are the main contributors to greenhouse gas emissions in food production and how can they be reduced?
...
3.  **Data Preparation**
    ...
    write.csv(df, file = "./02_tidy_data/cleaned_data.csv")
    ```
...
4.  **Analysis and Visualisation**
    ...
    **Data visualisation of the top ten total emissions (Kg CO2) by food products**
    ...
    # Sort by total emissions in descending order and limit to the top 10
    food_product_data <- food_product_data %>%
      arrange(desc(total_emissions)) %>%  # Sort by total emissions
      head(10)  # Get top 10 after sorting

    # Visualise the data  
    ggplot(food_product_data, aes(x = food_product, y = total_emissions, fill = food_product)) +
      geom_bar(stat = "identity", position = "dodge") +
      labs(title = "Top Ten Total Emissions (Kg CO2) by Food Product", x = "Food Product", y
    ...
    **Data visualisation to investigate the cause of the outlier in greenhouse gas emissions box plot**
    ...
    ggplot(plant_based_product, aes(x = food_product, y = greenhouse_gas_emissions_per_100g_protein_kg_co_eq_per_100g_protein)) + 
    ...
    **Scatter Plot on Eutrophication (g PO4eq per 100g protein) versus Greenhouse Gas Emissions (kg CO₂-eq per 100g protein)**
    ...
    \
    **Supporting Research\
    **Higher levels of eutrophication increases greenhouse gas emissions to the atmosphere, the emission of CH~4~ have been observed to increase with eutrophication (DelSontro et al., 2018).
    ...
    **Calculating the efficiency of environmental impact across all food products by 75%**
    ...
    df <- df %>% 
      mutate(
        reduced_eutrophication = eutrophying_emissions_per_100g_protein_g_po_eq_per_100_grams_protein * (1 - eutrophication_reduction_percentage / 100),
        eutrophication_reduction_amount = eutrophying_emissions_per_100g_protein_g_po_eq_per_100_grams_protein* (eutrophication_reduction_percentage / 100)
      )

    # Calculate total reduction
    total_original <- sum(df$eutrophying_emissions_per_100g_protein_g_po_eq_per_100_grams_protein)

    total_reduced <- sum(df$reduced_eutrophication)

    total_reduction <- sum(df$eutrophication_reduction_amount)

    # Print Summary Statistics 
    cat("Total Original Eutrophication:", total_original, "g PO₄eq\n")
    cat("Total Reduced Eutrophication:", total_reduced, "g PO₄eq\n")
    cat("Total Reduction:", total_reduction, "g PO₄eq\n")
    cat("Overall Reduction Efficiency:", (total_reduction / total_original) * 100,"%\n")

    ```

    **Visualisation: Before versus After Reduction**

    ```{r}
    ggplot(df, aes(x = reorder(food_product, -eutrophying_emissions_per_100g_protein_g_po_eq_per_100_grams_protein))) +
      geom_bar(aes(y = eutrophying_emissions_per_100g_protein_g_po_eq_per_100_grams_protein, fill = "Original"), stat = "identity", alpha = 0.7) +
      geom_bar(aes(y = reduced_eutrophication, fill = "Reduced"), stat = "identity", alpha = 0.7) +
      labs(title = "Eutrophication Impact Before and After Reduction",
           x = "Food Product",
           y = "Eutrophication ((g PO₄eq per 100g protein)"
           ) + 
      theme_minimal() +
       theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      scale_fill_manual(values = c("Original" = "red", "Reduced" = "blue"))
      
    ```

    This visualisation displays the eutrophication impact before and after reduction by 75% for various food products. From the chart, Animal-based product dominates the eutrophication impact, that is Beef (dairy herd), Beef (beef herd) and farmed fish have the highest eutrophication values. The reduction (blue portion) still leaves them higher than plant-based product.\
    \
    The chart also shows some plant-based products having notable eutrophication reduction impact. Dark chocolate, Coffee, Tomatoes, Berries & Grapes and Rice show substantial eutrophication values, although they are generally lower than the animal products.\
    \
    The reduction effect by 75% is proportional due to the fact that, high-impact food shows a more noticeable reduction, but they still maintain a significant eutrophication footprint.\
    \
    **Calculating the efficiency of freshwater use across food products by 15%**

    ```{r}
    # Define a percentage 
    freshwater_reduction_percentage <- 60

    # Apply freshwater reduction
    df <- df %>% 
      mutate(
        reduced_freshwater = freshwater_withdrawals_per_1000kcal_liters_per_1000kcal * (1- freshwater_reduction_percentage / 100),
        freshwater_reduction_amount = freshwater_withdrawals_per_1000kcal_liters_per_1000kcal * (freshwater_reduction_percentage / 100)
      )

    # Calculate total reduction
    total_original_freshwater_use <- sum(df$freshwater_withdrawals_per_1000kcal_liters_per_1000kcal)

    total_reduced_freshwater_use <- sum(df$reduced_freshwater)

    total_reduction_freshwater_use <- sum(df$freshwater_reduction_amount)

    # Print Summary Statistics
    print(paste("Total Original Freshwater Use:", round(total_original_freshwater_use, 2), "litres per 1000kcal"))

    print(paste("Total Reduced Freshwater Use:", round(total_reduced_freshwater_use, 2), "litres per 1000kcal"))

    print(paste("Total Reduction in Freshwater Use:", round(total_reduction_freshwater_use, 2), "litres per 1000kcal"))
    ```

    **Visualisation before and after reduction**

    ```{r}
    ggplot(df, aes(x = reorder(food_product, -freshwater_withdrawals_per_1000kcal_liters_per_1000kcal))) +
      geom_bar(aes(y= freshwater_withdrawals_per_1000kcal_liters_per_1000kcal, fill = "Original"), stat = "identity", alpha = 0.7) +
      geom_bar(aes(y = reduced_freshwater, fill = "Reduced"), stat = "identity", alpha=0.7) +
      labs(title = "Freshwater Withdrawal Before and After Reduction",
           x = "Food Product",
           y = "Freshwater Withdrawal (litres per 1000kcal)"
           ) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      scale_fill_manual(values = c("Original" = "blue", "Reduced" = "lightblue"))
...
5.  **Recommendations & Conclusion\
    ...
    **Policy-makers should invest in alternative protein development for a secure, sustainable food supply. Supporting alternative protein innovation and commercialisation will increase food security while reducing environmental impacts (Global Food Institute, 2025).\
    Developing and implementing policies and technologies to reduce emissions from various sectors is crucial.
