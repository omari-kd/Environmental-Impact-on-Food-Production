...
4.  **Analysis and Visualisation**
    ...
    **Data visualisation of plant-based and animal-based on land use (m² per 100g protein) versus greenhouse gas emissions (kg CO2-eq per 100g protein)**
    ...
    The animal-based food generally do have lower land use. The trend line is relatively flat, suggesting that increasing land use does not significantly increase emissions within this category.

    The plant-based food show a wider range of land use, with one food product using significantly more land. The trend line is much steeper, indicating that higher land use is strongly associated with higher emission in this category.
    ...
    **Data Visualisation on top freshwater consuming food products litres per 1000kcal**
    ...
    ggplot(top_water_food, aes(x = reorder(food_product, freshwater_withdrawals_per_1000kcal_liters_per_1000kcal), y = freshwater_withdrawals_per_1000kcal_liters_per_1000kcal, fill = category)) + 
      geom_bar(stat = "identity") +
      coord_flip() +
      labs(title = "Top 10 Water-Consuming food",
           x = "Food Product",
    ...
    **Investigating the Outliers Further in Freshwater Withdrawals (litres per 1000 kcal)**
    ...
    ggplot(water_outliers, aes(x = reorder(food_product, freshwater_withdrawals_per_1000kcal_liters_per_1000kcal), 
      ...
      geom_col() +
      coord_flip() +
      labs(title = "Outliers in Freshwater Withdrawals (litres per 1000kcal)", 
    ...
    **Box plot on plant-based versus animal-based scarcity- weighted use litres per 1000kcal**
    ...
    ggplot(df, aes(x = category, scarcity_weighted_water_use_per_1000kcal_liters_per_1000_kilocalories  , fill = category)) +
      geom_boxplot(alpha = 0.7) +
      labs(title = "Scarcity Water Use by Food Category",
           x = "Food Category",
...
5.  **Recommendations & Conclusion**
    ...
    **Strategies for freshwater use**

    A shift towards a plant-based diet helps alleviate the strain on freshwater resources. Conserving water by adopting a plant-based diet can contribute to the preservation of freshwater ecosystems and ensure adequate supplies for future generation. By embracing a plant-based diet, you can make a positive impact on water conservation efforts and play a role in mitigating the environmental challenges associated with water scarcity (Nussli118, n.d.).

    Also Good Food Institute (n.d) stated that plant-based meat production uses 72-99 percent less water and 47-99 percent less land and causes 51-91 percent less water pollution and emits 30-90 percent less greenhouse gas emissions.
:::
...
14. Finnigan, T. (2011). *The nutritional properties of mycoprotein are essential for meat alternative development*. ScienceDirect. Retrieved from [https://www.sciencedirect.com/science/article/abs/pii/S0308814624031868#:\~:text=The%20nutritional%20properties%20of%20mycoprotein%20are%20essential,the%20structure%20of%20meat%20myofibers%20(Finnigan%2C%202011).](https://www.sciencedirect.com/science/article/abs/pii/S0308814624031868#:~:text=The%20nutritional%20properties%20of%20mycoprotein%20are%20essential,the%20structure%20of%20meat%20myofibers%20(Finnigan%2C%202011).)
